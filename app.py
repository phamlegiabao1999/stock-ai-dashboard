...
# --- 5. XỬ LÝ DỮ LIỆU (FIX RATE LIMIT) ---
if (btn_analyze or st.session_state.data is not None) and ma_input:
    # Chỉ tải lại khi nhấn nút phân tích hoặc đổi mã mới
        ...
        with st.spinner(f'Đang tải dữ liệu {ma_input}...'):
            try:
                ...
                if not df.empty:
                    ...
                    st.error("Không tìm thấy dữ liệu. Thử lại sau ít phút!")
            except Exception as e:
                st.error(f"Lỗi tải dữ liệu: {e}. Có thể bạn đã thao tác quá nhanh!")
    ...
    if st.session_state.data is not None:
        ...
        if prompt := st.chat_input("Hỏi AI..."):
            ...
            with st.chat_message("assistant"):
                try:
                    # RẤT QUAN TRỌNG: KHÔNG dùng 'models/'
