...
if btn_analyze or "main_df" in st.session_state:
    ...
    if df is not None and not df.empty:
        ...
        # --- PHẦN SO SÁNH (ĐÃ FIX LỖI VALUEERROR) ---
        if st.session_state.compare_df is not None and not st.session_state.compare_df.empty:
            st.markdown("---")
            ...
            # Đồng bộ hóa dữ liệu (Merge) để tránh lỗi lệch ngày hoặc dữ liệu trống
            combined = pd.merge(df[['Close']], df_ss[['Close']], left_index=True, right_index=True, how='inner', suffixes=('_chinh', '_ss'))
            
            # Tính % tăng trưởng từ mốc đầu tiên
            combined['Perf_Chinh'] = (combined['Close_chinh'] / combined['Close_chinh'].iloc[0] - 1) * 100
            combined['Perf_SS'] = (combined['Close_ss'] / combined['Close_ss'].iloc[0] - 1) * 100
            
            # Vẽ biểu đồ so sánh
            st.line_chart(combined[['Perf_Chinh', 'Perf_SS']].rename(columns={
                'Perf_Chinh': st.session_state.name_chinh,
                'Perf_SS': st.session_state.name_ss
            }))
            st.info("💡 Biểu đồ so sánh mức sinh lời lũy kế (%) của 2 mã trong 1 năm qua.")
        ...
        with c2:
            ...
            plan = pd.DataFrame({
                "Vị thế": ["Mua mới", "Cắt lỗ"],
                "Giá tham khảo": [f"Quanh {lw_ht:,.0f}", f"Dưới {lw_ht*0.97:,.0f}"]
            })
            st.table(plan)
