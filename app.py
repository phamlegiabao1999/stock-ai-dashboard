...
if (btn_analyze or st.session_state.data is not None) and ma_input:
    if btn_analyze or st.session_state.ma_current != ma_input:
        # Xử lý mã .VN cho chứng khoán Việt Nam
    ...
    if st.session_state.data is not None:
        ...
        # --- 5. CHAT AI (SỬA LỖI 404 TUYỆT ĐỐI) ---
        st.markdown("---")
        ...
        if prompt := st.chat_input("Hỏi AI..."):
            ...
            with st.chat_message("assistant"):
                try:
                    # KHÔNG dùng tiền tố models/, CHỈ dùng tên model trực tiếp
                    response = client.models.generate_content(
                        model="gemini-1.5-flash",
                        contents=f"Dữ liệu mã {st.session_state.ma_current}: Giá {g_ht:,.0f}, RSI {rsi_ht:.2f}. {prompt}"
                    )
                    st.markdown(response.text)
                    st.session_state.messages.append({"role": "assistant", "content": response.text})
                except Exception as e:
                    # Gỡ lỗi chi tiết nếu vẫn thất bại
                    st.error(f"Lỗi AI: {e}")
