...
import pandas as pd
import numpy as np
from google import genai  # Thư viện chuẩn mới bạn vừa cài
...
if (btn_analyze or st.session_state.data is not None) and ma_input:
    ...
    if st.session_state.data is not None:
        ...
        # Lấy giá trị item() để tránh lỗi Series
        ...
        with col_l:
            st.subheader("📚 Công thức & Nhận định")
            with st.expander("Chi tiết toán học"):
                st.write(f"MA20: {ma_ht:,.0f} | RSI: {rsi_ht:.2f}")
        ...
        # --- 6. CHAT AI (SỬA LỖI 404 TRIỆT ĐỂ) ---
        st.markdown("---")
        ...
        if prompt := st.chat_input("Hỏi gì đó..."):
            ...
            with st.chat_message("assistant"):
                try:
                    # RẤT QUAN TRỌNG: KHÔNG dùng 'models/', CHỈ dùng 'gemini-1.5-flash'
                    response = client.models.generate_content(
                        model="gemini-1.5-flash",
                        contents=f"Mã chứng khoán {st.session_state.ma_current}, Giá {g_ht:,.0f}, RSI {rsi_ht:.2f}. Trả lời câu hỏi: {prompt}"
                    )
                    st.markdown(response.text)
                    st.session_state.messages.append({"role": "assistant", "content": response.text})
                except Exception as e:
                    # Hiển thị lỗi sạch để debug
                    st.error(f"Lỗi AI: {e}")
